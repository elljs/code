...
export default function CopilotView() {
	...
	return (
		<div className="h-full w-full flex flex-col justify-between overflow-hidden">
			...
			<ChatMessageList
				ref={contentRef}
				className="flex-1 overflow-auto no-scrollbar px-1"
			>
				<ChatBubble
					variant="sent"
					onClick={async () => {
						state.isLoading = true;
						const content = "帮我实现通过微信消息调整量化策略参数";
				...
				>
					...
					<ChatBubbleMessage variant="sent">
						<span className="text-sky-500">@楚留香</span>{" "}
						帮我实现通过微信消息调整量化策略参数
					</ChatBubbleMessage>
				</ChatBubble>
					...
					<ChatBubble variant="received">
						...
						<ChatBubbleMessage variant="received">
							{state.isLoading ? (
								...
								<MarkdownRenderer>{state.message}</MarkdownRenderer>
							)}
						</ChatBubbleMessage>
					</ChatBubble>
				)}
