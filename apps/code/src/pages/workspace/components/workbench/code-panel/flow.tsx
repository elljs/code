...
const FlowNode = ({ id, name, icon }: FlowNodeProps) => {
    return (
        <div key={id} className="flex justify-between items-center h-10 w-full bg-card p-2 rounded-md hover:bg-primary cursor-grab">
...
const initialNodes = [
    { id: 'start', type: 'StartFlowNode', position: { x: 335, y: 200 }, data: { label: '开始' } },
    { id: '1-1', position: { x: 200, y: 400 }, data: { label: '抓取财经网站数据' } },
    ...
    { id: 'end', type: 'EndFlowNode', position: { x: 335, y: 900 }, data: { label: '结束' } },
];
const initialEdges = [
